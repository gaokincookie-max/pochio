...
function collisionContext(a,b,pair){
 ...
 return {id:eventSerial++,time:now,a,b,primary:a,other:b,x:(a.position.x+b.position.x)/2,y:(a.position.y+b.position.y)/2,av,bv,relativeSpeed:rv,headOn:Math.abs(relDot)>.62,shallow:Math.abs(relDot)<.34,near,nearCount:near.length,groupA:getGroup(a),groupB:getGroup(b),preColorsA:new Set(a.pocho.colorsSeen),preColorsB:new Set(b.pocho.colorsSeen),preContactCountA:(a.pocho.contacts.get(b.pocho.id)?.count||0),preContactCountB:(b.pocho.contacts.get(a.pocho.id)?.count||0),preTotalContactsA:a.pocho.contactCount,preTotalContactsB:b.pocho.contactCount,preGA:getGroup(a).length,preGB:getGroup(b).length,prevContactIdA:a.pocho.lastContactId,prevContactIdB:b.pocho.lastContactId,preLastContactAtA:a.pocho.lastContactAt,preLastContactAtB:b.pocho.lastContactAt,prevPushedByA:a.pocho.lastPushedBy,prevPushedByB:b.pocho.lastPushedBy,prevPushAtA:a.pocho.lastPushAt,prevPushAtB:b.pocho.lastPushAt,trigger:null,victims:[],causeType:'collision',pair};
}
...
function emotionMatch(rule,b,ctx,eventType){const p=b.pocho,now=ctx?.time||performance.now(),q=ctx?.otherForEmotion?.pocho||ctx?.other?.pocho||null,other=ctx?.otherForEmotion||ctx?.other||null,rec=other?contactRec(b,other):null,g=getGroup(b),neighbors=bondedNeighbors(b);switch(rule.id){
 ...
 case'emotion-4':return eventType==='contact'&&ctx?.trigger==='何も起こらない'&&ctx.relativeSpeed>=8&&ctx.prevContactIdForEmotion&&ctx.prevContactIdForEmotion!==other?.pocho.id&&now-(ctx.prevContactAtForEmotion||0)<=2000&&q&&q.color!==p.color&&((ctx.prevPushedByForEmotion&&ctx.prevPushedByForEmotion!==q.id)||p.lastPushedBy&&p.lastPushedBy!==q.id);
 ...
 case'emotion-6':return eventType==='pop'&&ctx?.nearWitness?.has(b)&&ctx.root&&!!p.contacts.get(ctx.root.pocho.id)?.everStuck&&now-(p.contacts.get(ctx.root.pocho.id)?.lastAt||0)<=5000&&!ctx.victims.includes(b);
 ...
 case'emotion-8':return eventType==='periodic'&&g.length>=5&&new Set(g.map(x=>x.pocho.color)).size>=4&&now-p.lastGroupJoinAt>=3000&&now-p.lastGroupPopAt>=3000&&p.stickCount>=3&&neighbors.length>=2;
