...
const LAUNCH_ZONE_TOP_RATIO=.56; // v0.2.6: launch zone enlarged upward for wider swings
let save=loadSave();
...
let feverActive=false,feverUntil=0,feverCooldownUntil=0,feverTriggeredRules=new Set(),feverLastRule=null;
const FEVER_DURATION_MS=8000,FEVER_COOLDOWN_MS=5000,FEVER_SCORE_MULT=1.5;
let currentSet=null,nextSetDesc=null,grabbed=null,grabConstraint=null,pointerHistory=[],activePointerId=null;
let bodies=new Set(), bonds=new Set(), lastActivity=0, settleSince=0, paused=false, gameStartedAt=0, finalShotAt=0;
const FINAL_WATCH_MAX_MS=8000;
const POP_WARN_MS=200, INDUCED_STEP_MS=110, POP_FLASH_MS=180;
let runStats=null, recentEvents=[], eventSerial=1;
let secretTriggered=new Set(),secretState={arkSince:0,pendingSurvivor:null,lastBoardSampleAt:0,bodyTimeline:[]},secretDisplayUntil=0;

function defaultSpecial(){return {maxChain:0,maxSingleScore:0,maxFever:0,maxGroupSize:0,maxRoles:0,maxSticks:0,maxPops:0}}
function defaultSave(){return {saveVersion:2,meta:{createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),lastGameVersion:'0.3.3'},profile:{playerId:null,playerName:null,registeredAt:null,accountType:'local',icon:{color:'みずいろ',expression:'ごきげん',sizeClass:'中',decoration:'なし'}},best:{short:0,middle:0,long:0},plays:{short:0,middle:0,long:0},totals:{shots:0,contacts:0,sticks:0,pops:0,induced:0,score:0,fevers:0,roles:0},roles:{},records:{specialBest:{short:defaultSpecial(),middle:defaultSpecial(),long:defaultSpecial()},bestAt:{short:null,middle:null,long:null},daily:{date:'',best:{short:0,middle:0,long:0},bestAt:{short:null,middle:null,long:null}},monthly:{month:'',best:{short:0,middle:0,long:0},bestAt:{short:null,middle:null,long:null}},weekly:{weekId:'',mode:'',metric:'',bestValue:0,bestAt:null}}}}
function normalizeSave(s){const d=defaultSave(),o=s||{};return Object.assign(d,o,{saveVersion:2,meta:Object.assign(d.meta,o.meta||{}),profile:Object.assign(d.profile,o.profile||{},{icon:Object.assign(d.profile.icon,o.profile?.icon||{})}),best:Object.assign(d.best,o.best||{}),plays:Object.assign(d.plays,o.plays||{}),totals:Object.assign(d.totals,o.totals||{}),roles:o.roles||{},records:Object.assign(d.records,o.records||{},{specialBest:{short:Object.assign(defaultSpecial(),o.records?.specialBest?.short||{}),middle:Object.assign(defaultSpecial(),o.records?.specialBest?.middle||{}),long:Object.assign(defaultSpecial(),o.records?.specialBest?.long||{})},bestAt:Object.assign(d.records.bestAt,o.records?.bestAt||{}),daily:Object.assign(d.records.daily,o.records?.daily||{},{best:Object.assign(d.records.daily.best,o.records?.daily?.best||{}),bestAt:Object.assign(d.records.daily.bestAt,o.records?.daily?.bestAt||{})}),monthly:Object.assign(d.records.monthly,o.records?.monthly||{},{best:Object.assign(d.records.monthly.best,o.records?.monthly?.best||{}),bestAt:Object.assign(d.records.monthly.bestAt,o.records?.monthly?.bestAt||{})}),weekly:Object.assign(d.records.weekly,o.records?.weekly||{})})})}
function loadSave(){try{return normalizeSave(JSON.parse(localStorage.getItem(SAVE_KEY)||'null'))}catch{return defaultSave()}}
function persist(){try{save.meta.updatedAt=new Date().toISOString();save.meta.lastGameVersion='0.3.3';localStorage.setItem(SAVE_KEY,JSON.stringify(save));window.dispatchEvent(new CustomEvent('pocho:save-changed',{detail:{save:JSON.parse(JSON.stringify(save))}}))}catch{}}
...
function teardownPhysics(){if(grabConstraint&&engine){try{Composite.remove(engine.world,grabConstraint)}catch{}}grabConstraint=null;activePointerId=null;dragging=false;if(render){Render.stop(render);render.canvas.remove();render=null}if(runner){Runner.stop(runner);runner=null}engine=null;walls=[];topWall=null;bodies.clear();bonds.clear();current=null;currentSet=null;grabbed=null;pointerHistory=[]}
...
function startGame(m){
 stopGame();const session=++gameSessionId;modeKey=m;score=0;remaining=MODES[m].shots;shotIndex=0;lastActivity=performance.now();settleSince=0;finalShotAt=0;gameStartedAt=performance.now();recentEvents=[];feverActive=false;feverUntil=0;feverCooldownUntil=0;feverTriggeredRules=new Set();feverLastRule=null;secretTriggered=new Set();secretState={arkSince:0,pendingSurvivor:null,lastBoardSampleAt:0,bodyTimeline:[]};secretDisplayUntil=0;setFeverUi(false);
 runStats={contacts:0,sticks:0,pops:0,induced:0,newRoles:new Set(),maxRarity:0,maxSingle:0,rolesTriggered:0,shots:0,fevers:0,maxChain:0,maxGroupSize:1};
 showScreen('game');setupPhysics();nextSetDesc=makeSetDescriptor();nextDesc=null;updateHud();drawNext();
 window.dispatchEvent(new CustomEvent('pocho:run-started',{detail:{mode:modeKey,expectedShots:MODES[modeKey].shots,clientStartedAt:new Date().toISOString()}}));
...
function pointerStart(e){
 // v0.2.6: one physical grab may be controlled by exactly one pointer.
 // Additional fingers are ignored so they can never leave a second stale Matter constraint behind.
 if(paused||!currentSet||currentSet.launched||dragging||activePointerId!==null)return;
 const p=point(e);let target=null,best=Infinity;
 ...
 if(!target)return;
 if(grabConstraint){try{Composite.remove(engine.world,grabConstraint)}catch{}grabConstraint=null}
 activePointerId=e.pointerId;dragging=true;grabbed=target;current=target;anchor={x:target.position.x,y:target.position.y};pointerHistory=[];
...
function pointerMove(e){
 if(e.pointerId!==activePointerId||!dragging||!currentSet||!grabConstraint||!grabbed)return;const p=clampToLaunchZone(point(e),grabbed.pocho.radius);
...
function pointerEnd(e){
 ...
 if(!dragging||!currentSet||currentSet.launched){dragging=false;if(grabConstraint&&engine){try{Composite.remove(engine.world,grabConstraint)}catch{}grabConstraint=null}grabbed=null;pointerHistory=[];return}
 dragging=false;
 const set=currentSet,now=performance.now();
 if(grabConstraint){Composite.remove(engine.world,grabConstraint);grabConstraint=null}try{render.canvas.releasePointerCapture?.(e.pointerId)}catch{}
 ...
 if(remaining>0)setTimeout(()=>{if(state==='game'&&!currentSet)spawnCurrent()},500);else{finalShotAt=now;gameStatus.textContent='最後の3ぽちょを見届けています（最大8秒）'}e.preventDefault();
}
...
function contactRec(a,b){let r=a.pocho.contacts.get(b.pocho.id);if(!r){r={count:0,lastAt:0,firstAt:0,everStuck:false,stickCount:0,detachCount:0,lastStickAt:0,lastDetachAt:0,lastStickExpression:null,firstStickOtherExpression:null,lastStickWasSelfSingle:false,maxSpeed:0,recentSpeeds:[]};a.pocho.contacts.set(b.pocho.id,r)}return r}
function collisionContext(a,b,pair){
 ...
 return {id:eventSerial++,time:now,a,b,primary:a,other:b,x:(a.position.x+b.position.x)/2,y:(a.position.y+b.position.y)/2,av,bv,relativeSpeed:rv,headOn:Math.abs(relDot)>.62,shallow:Math.abs(relDot)<.34,near,nearCount:near.length,groupA:getGroup(a),groupB:getGroup(b),preColorsA:new Set(a.pocho.colorsSeen),preColorsB:new Set(b.pocho.colorsSeen),preContactCountA:(a.pocho.contacts.get(b.pocho.id)?.count||0),preContactCountB:(b.pocho.contacts.get(a.pocho.id)?.count||0),preTotalContactsA:a.pocho.contactCount,preTotalContactsB:b.pocho.contactCount,preGA:getGroup(a).length,preGB:getGroup(b).length,prevContactIdA:a.pocho.lastContactId,prevContactIdB:b.pocho.lastContactId,preLastContactAtA:a.pocho.lastContactAt,preLastContactAtB:b.pocho.lastContactAt,prevPushedByA:a.pocho.lastPushedBy,prevPushedByB:b.pocho.lastPushedBy,prevPushAtA:a.pocho.lastPushAt,prevPushAtB:b.pocho.lastPushAt,prevPushImpactA:a.pocho.lastPushImpact||0,prevPushImpactB:b.pocho.lastPushImpact||0,trigger:null,victims:[],causeType:'collision',pair};
}
function collisionStart(ev){if(state!=='game'||paused||initializingBoard)return;const now=performance.now();for(const pair of ev.pairs){const a=pair.bodyA,b=pair.bodyB;if(!a.pocho||!b.pocho||!a.pocho.launched||!b.pocho.launched||a.pocho.pendingPop||b.pocho.pendingPop)continue;if(isBonded(a,b))continue;if(a.pocho.launchSetId&&a.pocho.launchSetId===b.pocho.launchSetId&&now-Math.max(a.pocho.launchedAt||0,b.pocho.launchedAt||0)<450)continue;processContact(a,b,pair)}}
function processContact(a,b,pair){
 ...
 for(const [self,other] of [[a,b],[b,a]]){const r=contactRec(self,other);(r.recentSpeeds||(r.recentSpeeds=[])).push(ctx.relativeSpeed);if(r.recentSpeeds.length>3)r.recentSpeeds.shift();if(ctx.relativeSpeed>=T.HIGH)self.pocho.lastHighSpeedAt=now}
 const prevA={id:ctx.prevContactIdA,at:ctx.preLastContactAtA,pushed:ctx.prevPushedByA},prevB={id:ctx.prevContactIdB,at:ctx.preLastContactAtB,pushed:ctx.prevPushedByB};
 updatePushProvenance(a,b,ctx.relativeSpeed);
 ...
 if(a.pocho&&b.pocho){ctx.otherForEmotion=b;ctx.prevContactIdForEmotion=prevA.id;ctx.prevContactAtForEmotion=prevA.at;ctx.prevPushedByForEmotion=prevA.pushed;ctx.prevPushAtForEmotion=ctx.prevPushAtA;ctx.prevPushImpactForEmotion=ctx.prevPushImpactA||0;runEmotionRules('contact',ctx,[a]);ctx.otherForEmotion=a;ctx.prevContactIdForEmotion=prevB.id;ctx.prevContactAtForEmotion=prevB.at;ctx.prevPushedByForEmotion=prevB.pushed;ctx.prevPushAtForEmotion=ctx.prevPushAtB;ctx.prevPushImpactForEmotion=ctx.prevPushImpactB||0;runEmotionRules('contact',ctx,[b])}
 // 「第三者に押された」は、単に触れただけではなく速度差のある衝突だけを履歴化する。
 if(a.pocho&&b.pocho){const sa=speed(a),sb=speed(b),rv=ctx.relativeSpeed;if(rv>=4.5&&sb>sa+.75){a.pocho.lastPushedBy=b.pocho.id;a.pocho.lastPushAt=now;a.pocho.lastPushImpact=rv}if(rv>=4.5&&sa>sb+.75){b.pocho.lastPushedBy=a.pocho.id;b.pocho.lastPushAt=now;b.pocho.lastPushImpact=rv}}
...
function bodyById(id){for(const b of bodies)if(b.pocho?.id===id)return b;return null}
function bondedNeighbors(b){const out=[];for(const c of bonds){if(c.bodyA===b&&c.bodyB?.pocho)out.push({body:c.bodyB,bond:c});else if(c.bodyB===b&&c.bodyA?.pocho)out.push({body:c.bodyA,bond:c})}return out}
function sizeRank(x){return x==='小'?0:x==='中'?1:2}
function uniqueGroupCount(p){return new Set((p.groupsHistory||[]).map(x=>x.join('|'))).size}
function pruneTimes(a,now,ms){while(a.length&&now-a[0]>ms)a.shift();return a}
function applyEmotionEffect(b,rule,now){if(!b?.pocho||!rule?.active)return false;const p=b.pocho;if(now-p.lastExpressionChange<4000||p.emotionRuleHistory?.has(rule.id))return false;const before=p.expression,i=EXPRESSIONS.indexOf(before);let after=before;if(rule.effect==='+1')after=EXPRESSIONS[Math.max(0,i-1)];else if(rule.effect==='-1')after=EXPRESSIONS[Math.min(EXPRESSIONS.length-1,i+1)];else if(rule.effect==='SET_HAPPY')after='ごきげん';else if(rule.effect==='SET_NEUTRAL')after='ふつう';else if(rule.effect==='SET_GRUMPY')after='不機嫌';else if(rule.effect==='FLIP')after=before==='ごきげん'?'不機嫌':before==='不機嫌'?'ごきげん':'ふつう';if(after===before)return false;p.expression=after;p.expressionChanges++;p.lastExpressionChange=now;p.expressionsSeen.add(after);p.lastEvent='expressionChange';p.lastEventAt=now;(p.emotionRuleHistory||(p.emotionRuleHistory=new Set())).add(rule.id);pushRecent({type:'表情変化',time:now,x:b.position.x,y:b.position.y,bodyIds:[p.id],emotionRule:rule.id,before,after});return true}
function emotionMatch(rule,b,ctx,eventType){const p=b.pocho,now=ctx?.time||performance.now(),q=ctx?.otherForEmotion?.pocho||ctx?.other?.pocho||null,other=ctx?.otherForEmotion||ctx?.other||null,rec=other?contactRec(b,other):null,g=getGroup(b),neighbors=bondedNeighbors(b);switch(rule.id){
 case'emotion-1':return eventType==='stick'&&other&&rec.stickCount>=2&&rec.detachCount>=1&&rec.lastDetachAt&&now-rec.lastDetachAt>=3000&&q.expression==='ごきげん';
 case'emotion-2':{if(eventType!=='periodic'||g.length>3)return false;for(const n of neighbors){const r=contactRec(b,n.body),age=now-(n.bond.pochoBond?.createdAt||now);if(age>=5000&&r.count>=5&&now-p.lastHighSpeedAt>=5000&&now-n.body.pocho.lastHighSpeedAt>=5000&&p.behaviorCandidateHits===0&&n.body.pocho.behaviorCandidateHits===0)return true}return false}
 case'emotion-3':return eventType==='pop'&&ctx?.inducedFailed?.has(b)&&ctx.victims?.length>1&&(now-p.bornAt)>=5000;
 case'emotion-4':{if(!(eventType==='contact'&&ctx?.trigger==='何も起こらない'&&ctx.relativeSpeed>=12&&other&&rec.count===1&&q&&q.color!==p.color&&ctx.prevPushedByForEmotion&&ctx.prevPushedByForEmotion!==q.id&&now-(ctx.prevPushAtForEmotion||0)<=500&&(ctx.prevPushImpactForEmotion||0)>=8&&now-(p.wall.lastAt||0)>500))return false;const pusher=bodyById(ctx.prevPushedByForEmotion);return !!pusher?.pocho&&pusher.pocho.color!==q.color;}
 case'emotion-5':{if(eventType!=='periodic'||!p.lastLeftGroupAt||now-p.lastLeftGroupAt<2000||now-p.lastContactAt<2000||p.lastLeftGroupSize<4||p.lastLeftGroupAt-p.lastLeftGroupJoinedAt<3000)return false;const alive=p.lastLeftGroupMembers.map(bodyById).filter(Boolean);return alive.some(x=>getGroup(x).length>=3)}
 case'emotion-6':{if(eventType!=='pop'||!ctx.root||ctx.victims.includes(b))return false;const rr=p.contacts.get(ctx.root.pocho.id);if(!rr?.everStuck||rr.detachCount<1||!rr.lastDetachAt||!rr.lastStickAt)return false;const dx=b.position.x-ctx.root.position.x,dy=b.position.y-ctx.root.position.y;return Math.hypot(dx,dy)<=70&&now-rr.lastDetachAt<=5000&&(rr.lastDetachAt-rr.lastStickAt)>=2000}
 case'emotion-7':return eventType==='contact'&&ctx?.trigger==='何も起こらない'&&other&&rec.count>=7&&rec.stickCount===0&&now-rec.firstAt>=6000&&(rec.recentSpeeds||[]).slice(-3).length===3&&(rec.recentSpeeds||[]).slice(-3).every(v=>v>=6);
 case'emotion-8':return eventType==='periodic'&&g.length>=7&&new Set(g.map(x=>x.pocho.color)).size>=5&&now-p.lastGroupJoinAt>=5000&&now-p.lastGroupPopAt>=5000&&p.stickCount>=3&&neighbors.length>=2;
 case'emotion-9':pruneTimes(p.popWitnessTimes,now,3000);return eventType==='periodic'&&p.popWitnessTimes.length>=2&&g.length===1&&now-p.bornAt>=8000&&now-p.lastStickAtSelf>=3000;
 case'emotion-10':return eventType==='stick'&&other&&rec.stickCount>=2&&rec.detachCount>=1&&uniqueGroupCount(p)>=3&&now-rec.firstAt>=10000&&rec.firstStickOtherExpression===q.expression;
 case'emotion-11':return eventType==='periodic'&&now-p.bornAt>=20000&&p.colorsSeen.size>=6&&uniqueGroupCount(p)>=3&&p.stickCount>=3&&p.detachCount>=2&&p.popWitnessed>=3&&p.expressionChanges===0;
 default:return false}}
function runEmotionRules(eventType,ctx,targets){const now=ctx?.time||performance.now(),rules=DATA.expressionChangeRules||[];for(const b of targets||[]){if(!b?.pocho||!bodies.has(b))continue;for(const rule of rules){if(!rule.active||rule.event!==eventType)continue;if(emotionMatch(rule,b,ctx,eventType)&&applyEmotionEffect(b,rule,now))break}}}
function periodicEmotionChecks(now){for(const b of bodies){if(!b.pocho?.launched||b.pocho.pendingPop||now-b.pocho.lastEmotionCheck<500)continue;b.pocho.lastEmotionCheck=now;runEmotionRules('periodic',{time:now},[b])}}
function inducedRuleApplies(r,b){const p=b.pocho;if(r.layer==='size')return r.attribute===p.sizeClass;if(r.layer==='color')return r.attribute===p.color;if(r.layer==='expression')return r.attribute===p.expression;if(r.layer==='decoration')return r.attribute===p.decoration;return r.layer==='special'}
function inducedMatch(rule,candidate,source,root,ictx){const p=candidate.pocho,s=source.pocho,r=root.pocho,now=ictx.time,rec=contactRec(candidate,source),bondAge=now-(ictx.bondCreatedAt||now),group=ictx.preGroup,degree=(ictx.adj.get(candidate)||[]).length,sourceDegree=(ictx.adj.get(source)||[]).length;switch(rule.id){
 case'chain-size-1':return p.sizeClass==='小'&&s.sizeClass==='大';case'chain-size-2':return p.sizeClass==='小'&&Math.abs(sizeRank(p.sizeClass)-sizeRank(s.sizeClass))>=2&&bondAge<1000;case'chain-size-3':return p.sizeClass==='大'&&s.sizeClass==='大'&&group.length>=5;
 case'chain-color-1':return p.color==='さくら'&&rec.detachCount>=1&&rec.stickCount>=2;case'chain-color-2':return p.color==='みずいろ'&&ictx.rootHighSpeed&&now-p.wall.lastAt<=1000;case'chain-color-3':return p.color==='たまご'&&[3,6].includes(p.lastJoinOrdinal)&&p.color!==s.color;case'chain-color-4':return p.color==='わかば'&&bondAge>=3000&&(ictx.priorInducedAttempts||0)===0&&p.color!==s.color;case'chain-color-5':{const ns=(ictx.adj.get(candidate)||[]).map(x=>x.body);return p.color==='ふじ'&&ns.length>=3&&ns.filter(x=>x.pocho.color!==p.color).length>ns.length/2}case'chain-color-6':return p.color==='あんず'&&ictx.rootCauseCollision&&now-p.lastHighSpeedAt<=1000;
 case'chain-expression-1':return p.expression==='ごきげん'&&s.expression==='ごきげん'&&bondAge>=2000;case'chain-expression-2':return p.expression==='ふつう'&&s.expression==='不機嫌'&&group.length>=4&&degree>=2;case'chain-expression-3':return p.expression==='不機嫌'&&s.expression==='不機嫌'&&bondAge<1000&&ictx.rootHighSpeed;case'chain-expression-4':return p.expression==='不機嫌'&&rec.stickCount===1&&p.color!==s.color&&rec.lastStickWasSelfSingle;
 case'chain-decoration-1':return p.decoration==='リボン'&&rec.stickCount>=2&&rec.detachCount>=1;case'chain-decoration-2':return p.decoration==='メガネ'&&p.lastJoinOrdinal%2===0&&p.contactCount%2===1&&p.color!==s.color;case'chain-decoration-3':{const maxR=Math.max(...group.map(x=>x.pocho.radius));return p.decoration==='王冠'&&p.radius<maxR-.5&&group.length>=6&&s.radius>=maxR-.5}case'chain-decoration-4':return p.decoration==='芽'&&now-p.bornAt>=6000&&p.stickCount>=3&&p.expressionChanges>0&&bondAge>=2000;
 case'chain-special-1':return p.color===s.color&&p.expression===s.expression&&p.sizeClass===s.sizeClass&&bondAge>=3000;case'chain-special-2':return ictx.depth===2&&group.length>=5&&p.color!==r.color&&p.expression!==r.expression&&p.decoration!==r.decoration;case'chain-special-3':{const colors=new Set(group.map(x=>x.pocho.color));const oldest=Math.min(...group.map(x=>x.pocho.bornAt));return colors.size>=5&&r.decoration!=='なし'&&p.decoration==='なし'&&p.bornAt===oldest}default:return false}}
function evaluateInduced(candidate,source,root,ictx){const order=['special','decoration','expression','color','size'];ictx.priorInducedAttempts=candidate.pocho.inducedAttempts||0;candidate.pocho.inducedAttempts++;for(const layer of order){for(const rule of DATA.inducedRules||[]){if(!rule.active||rule.layer!==layer||!inducedRuleApplies(rule,candidate))continue;if(inducedMatch(rule,candidate,source,root,ictx))return rule}}return null}
...
function checkClause(c,ctx,self=ctx.primary||ctx.a,other=ctx.other||(self===ctx.a?ctx.b:ctx.a)){
 ...
 if(c==='この相手とは一度もくっついていない')return !!rec&&(rec.stickCount||0)===0;
 if(c==='初接触から3秒以上経過')return !!rec&&!!rec.firstAt&&now-rec.firstAt>=3000;
...
function stick(a,b,ctx){
 ...
 for(const [x,y] of [[a,b],[b,a]]){const p=x.pocho,r=contactRec(x,y),wasSingle=getGroup(x).length===1;p.stickCount++;p.everStuck=true;p.aloneSince=performance.now();p.lastStickAtSelf=performance.now();r.everStuck=true;r.stickCount=(r.stickCount||0)+1;r.lastStickAt=performance.now();r.lastStickExpression=p.expression;r.lastStickWasSelfSingle=wasSingle;if(!r.firstStickOtherExpression)r.firstStickOtherExpression=y.pocho.expression;p.lastEvent='stick';p.lastEventAt=performance.now()}
 const before=ctx.groupA.length>=ctx.groupB.length?ctx.groupA:ctx.groupB;ctx.groupBefore=before;ctx.groupAfter=getGroup(a);ctx.newlyJoined=before.includes(a)?b:a;if(ctx.newlyJoined?.pocho){ctx.newlyJoined.pocho.lastGroupJoinAt=performance.now();ctx.newlyJoined.pocho.lastJoinOrdinal=ctx.groupAfter.length}recordGroups(ctx.groupAfter);runStats.maxGroupSize=Math.max(runStats.maxGroupSize||1,ctx.groupAfter.length);runStats.sticks++;save.totals.sticks++;lastActivity=performance.now();ctx.time=performance.now();ctx.otherForEmotion=b;runEmotionRules('stick',ctx,[a]);ctx.otherForEmotion=a;runEmotionRules('stick',ctx,[b]);
}
...
function breakBond(c){const a=c.bodyA,b=c.bodyB;if(!a?.pocho||!b?.pocho)return;const pre=getGroup(a),now=performance.now(),created=c.pochoBond?.createdAt||now;Composite.remove(engine.world,c);bonds.delete(c);for(const [x,y] of [[a,b],[b,a]]){x.pocho.detachCount++;x.pocho.aloneSince=now;x.pocho.lastEvent='detach';x.pocho.lastEventAt=now;x.pocho.lastLeftGroupAt=now;x.pocho.lastLeftGroupSize=pre.length;x.pocho.lastLeftGroupMembers=pre.filter(z=>z!==x).map(z=>z.pocho.id);x.pocho.lastLeftGroupJoinedAt=Math.min(x.pocho.lastGroupJoinAt||now,created);const r=contactRec(x,y);r.detachCount++;r.lastDetachAt=now}pushRecent({type:'detach',time:now,x:(a.position.x+b.position.x)/2,y:(a.position.y+b.position.y)/2})}
function popGroup(origin,ctx){
 const now=performance.now(),preGroup=getGroup(origin),adj=new Map();for(const b of preGroup)adj.set(b,[]);for(const c of bonds){if(adj.has(c.bodyA)&&adj.has(c.bodyB)){adj.get(c.bodyA).push({body:c.bodyB,bond:c});adj.get(c.bodyB).push({body:c.bodyA,bond:c})}}
 const victims=[origin],victimSet=new Set(victims),visited=new Set([origin]),failed=new Set(),queue=[{body:origin,depth:0}],inducedRules=[];ctx.primary=origin;ctx.root=origin;ctx.causeChain=buildCauseChain(origin);ctx.x=origin.position.x;ctx.y=origin.position.y;
 const rootHighSpeed=(ctx.relativeSpeed||0)>=T.HIGH,rootCauseCollision=ctx.causeType==='collision';
 while(queue.length){const cur=queue.shift();for(const edge of adj.get(cur.body)||[]){const cand=edge.body;if(visited.has(cand))continue;visited.add(cand);const ictx={time:now,preGroup,adj,depth:cur.depth+1,bondCreatedAt:edge.bond.pochoBond?.createdAt||now,rootHighSpeed,rootCauseCollision};const rule=evaluateInduced(cand,cur.body,origin,ictx);if(rule){victims.push(cand);victimSet.add(cand);inducedRules.push({body:cand,source:cur.body,rule,depth:cur.depth+1});queue.push({body:cand,depth:cur.depth+1})}else{failed.add(cand);cand.pocho.inducedSurvived++}}}
 ctx.victims=[...victims];ctx.inducedFailed=failed;ctx.inducedRules=inducedRules;const beforeNear=getNearby(ctx.x,ctx.y,T.NEAR*1.25,[]),remainingNear=beforeNear.filter(x=>!victimSet.has(x));ctx.densityWillResolve=beforeNear.length>=8&&remainingNear.length<=Math.max(3,beforeNear.length-victims.length);scoreEvent(ctx);
 for(const survivor of preGroup)if(!victimSet.has(survivor))survivor.pocho.lastGroupPopAt=now;
 const nearWitness=new Set();for(const witness of bodies){if(victimSet.has(witness)||!witness.pocho)continue;if(Math.hypot(witness.position.x-origin.position.x,witness.position.y-origin.position.y)<170){witness.pocho.popWitnessed++;(witness.pocho.popWitnessTimes||(witness.pocho.popWitnessTimes=[])).push(now);pruneTimes(witness.pocho.popWitnessTimes,now,10000);nearWitness.add(witness)}}ctx.nearWitness=nearWitness;
 ctx.survivorLostBonds=new Map();for(const survivor of preGroup){if(victimSet.has(survivor))continue;let n=0;for(const edge of adj.get(survivor)||[])if(victimSet.has(edge.body))n++;ctx.survivorLostBonds.set(survivor.pocho.id,n)}
 ctx.preBodyCount=[...bodies].filter(x=>x.pocho?.launched&&!x.pocho.pendingPop).length;ctx.postBodyCount=Math.max(0,ctx.preBodyCount-victims.length);ctx.victimSnapshot=victims.map(b=>({id:b.pocho.id,color:b.pocho.color,expression:b.pocho.expression,sizeClass:b.pocho.sizeClass,decoration:b.pocho.decoration}));
 runStats.pops++;runStats.maxChain=Math.max(runStats.maxChain||0,victims.length);runStats.induced+=Math.max(0,victims.length-1);save.totals.pops++;save.totals.induced+=Math.max(0,victims.length-1);
 // v0.2.5: POPを一瞬で消さず、起爆→誘爆の順に見えるよう時間差を付ける。
 const session=gameSessionId;
 for(const c of [...bonds])if(victimSet.has(c.bodyA)||victimSet.has(c.bodyB)){Composite.remove(engine.world,c);bonds.delete(c)}
 for(const b of victims){b.pocho.pendingPop=true;b.pocho.popAt=now+POP_WARN_MS;b.pocho.popWarningStart=b.pocho.popAt-POP_FLASH_MS;Body.setVelocity(b,{x:0,y:0});Body.setAngularVelocity(b,0);Body.setStatic(b,true);b.collisionFilter.mask=0}
 const depthByBody=new Map([[origin,0]]);for(const x of inducedRules)depthByBody.set(x.body,x.depth||1);
 for(const b of victims){const depth=depthByBody.get(b)||0,delay=POP_WARN_MS+depth*INDUCED_STEP_MS;b.pocho.popAt=now+delay;b.pocho.popWarningStart=b.pocho.popAt-POP_FLASH_MS;setTimeout(()=>{if(session!==gameSessionId||state!=='game'||!engine||!bodies.has(b))return;spawnPopFx(b,depth);Composite.remove(engine.world,b);bodies.delete(b);lastActivity=performance.now()},delay)}
 if(victims.length>1)setTimeout(()=>{if(session===gameSessionId&&state==='game')showChainPop(ctx.x,ctx.y,victims.length)},POP_WARN_MS);
 pushRecent({type:'popChain',time:now,x:origin.position.x,y:origin.position.y,victims:victims.length,induced:Math.max(0,victims.length-1),preBodyCount:ctx.preBodyCount,postBodyCount:ctx.postBodyCount,rootId:origin.pocho.id,colors:[...new Set(ctx.victimSnapshot.map(v=>v.color))]});
 lastActivity=now;ctx.time=now;runEmotionRules('pop',ctx,[...failed,...nearWitness]);checkSecretPopRoles(ctx,preGroup,victimSet);checkFever(now);
}
function buildCauseChain(b){const arr=[b],seen=new Set([b.pocho.id]);let id=b.pocho.lastPushedBy;for(let i=0;i<12&&id;i++){const n=[...bodies].find(x=>x.pocho.id===id);if(!n||seen.has(id))break;arr.unshift(n);seen.add(id);id=n.pocho.lastPushedBy}return arr}
...
function scoreEvent(ctx){
 ...
 matched.sort((a,b)=>b.rarity-a.rarity||b.score-a.score);const baseTotal=matched.reduce((s,r)=>s+r.score,0),rep=matched[0],total=Math.round(baseTotal*(feverActive?FEVER_SCORE_MULT:1));score+=total;save.totals.score+=total;runStats.rolesTriggered+=matched.length;save.totals.roles=(save.totals.roles||0)+matched.length;runStats.maxRarity=Math.max(runStats.maxRarity,rep.rarity);runStats.maxSingle=Math.max(runStats.maxSingle,total);
 let repNew=false;for(const r of matched){const key=String(r.id),entry=save.roles[key]||(save.roles[key]={discovered:false,count:0,best:0,first:null});if(!entry.discovered){entry.discovered=true;entry.first=new Date().toISOString();runStats.newRoles.add(r.id);if(r.id===rep.id)repNew=true}entry.count++;entry.best=Math.max(entry.best,total)}persist();showScorePop(ctx.x,ctx.y,rep,total,repNew);pushRecent({type:'score',time:performance.now(),score:total,x:ctx.x,y:ctx.y,maxRarity:rep.rarity,roleCount:matched.length,highRoleIds:matched.filter(r=>r.rarity>=4).map(r=>String(r.id))});updateHud();checkFever(performance.now());}
function liveBodies(){return [...bodies].filter(b=>b.pocho?.launched&&!b.pocho.pendingPop)}
function hasAll(set,items){return items.every(x=>set.has(x))}
function updatePushProvenance(a,b,rv){
 const sa=speed(a),sb=speed(b);let pusher=null,pushed=null;if(rv>=4.5&&sa>sb+.75){pusher=a;pushed=b}else if(rv>=4.5&&sb>sa+.75){pusher=b;pushed=a}else return;
 const pp=pusher.pocho,pd=pushed.pocho,root=pp.pushRootId||pp.id,base=(pp.pushTrace&&pp.pushTrace.length?pp.pushTrace:[pp.id]).slice(-11),trace=[...base,pd.id];pd.pushRootId=root;pd.pushTrace=trace;
 if(root===pd.id){const uniq=[...new Set(trace)];pd.pushCycleLen=uniq.length;const cols=new Set();for(const id of uniq){const x=bodyById(id);if(x?.pocho)cols.add(x.pocho.color)}pd.pushCycleColors=cols}
}
function forceSecretFever(role,now=performance.now()){
 if(feverActive&&now<feverUntil){feverUntil=now+FEVER_DURATION_MS;feverLastRule='secret:'+role.id;document.body.classList.add('fever-active');setFeverUi(true,now);pushRecent({type:'fever',time:now,rule:'secret:'+role.id,refresh:true});return}
 startFever({id:'secret:'+role.id,name:role.name},now);
}
function showSecretRole(role,points,isNew){
 secretDisplayUntil=performance.now()+2100;scoreLayer.querySelectorAll('.score-pop,.chain-pop').forEach(el=>el.remove());
 const el=document.createElement('div');el.className='secret-role-pop rarity-6';el.innerHTML=(isNew?'<span class="new">NEW</span>':'')+`<span class="secret-stars">★★★★★★</span><span class="role">${escapeHtml(role.name)}</span><span class="points">+${points.toLocaleString()}</span>`;scoreLayer.appendChild(el);setTimeout(()=>el.remove(),2000)
}
function awardSecret(id,meta={}){
 const role=(DATA.secretRoles||[]).find(r=>r.id===id);if(!role||!role.active||secretTriggered.has(id))return false;secretTriggered.add(id);const now=performance.now();forceSecretFever(role,now);
 const total=Math.round(role.score*(feverActive?FEVER_SCORE_MULT:1));score+=total;save.totals.score+=total;runStats.rolesTriggered++;save.totals.roles=(save.totals.roles||0)+1;runStats.maxRarity=Math.max(runStats.maxRarity,6);runStats.maxSingle=Math.max(runStats.maxSingle,total);
 const key=String(role.id),entry=save.roles[key]||(save.roles[key]={discovered:false,count:0,best:0,first:null});const isNew=!entry.discovered;if(isNew){entry.discovered=true;entry.first=new Date().toISOString();runStats.newRoles.add(role.id)}entry.count++;entry.best=Math.max(entry.best,total);persist();showSecretRole(role,total,isNew);pushRecent({type:'score',time:now,score:total,x:W/2,y:H*.35,maxRarity:6,roleCount:1,highRoleIds:[role.id],secret:true});updateHud();return true
}
function arkSnapshot(){const live=liveBodies(),combos=new Set(live.map(b=>b.pocho.color+'|'+b.pocho.expression)),sizes=new Set(live.map(b=>b.pocho.sizeClass)),decos=new Set(live.map(b=>b.pocho.decoration).filter(x=>x&&x!=='なし')),counts=new Map(COLORS.map(c=>[c.id,0]));for(const b of live)counts.set(b.pocho.color,(counts.get(b.pocho.color)||0)+1);return{live,combos,sizes,decos,minColor:Math.min(...counts.values())}}
function checkApocalypse(ctx){const now=performance.now(),r=recentEvents.filter(e=>now-e.time<=10000),sticks=r.filter(e=>e.type==='くっつく').length,pops=r.filter(e=>e.type==='popChain'),normalPops=pops.length,induced=pops.reduce((s,e)=>s+(e.induced||0),0),emotions=r.filter(e=>e.type==='表情変化').length,hi=new Set(r.filter(e=>e.type==='score').flatMap(e=>e.highRoleIds||[])),popColors=new Set(pops.flatMap(e=>e.colors||[])),timeline=secretState.bodyTimeline.filter(x=>now-x.time<=10000),start=timeline.length?timeline[0].count:0,end=ctx.postBodyCount??liveBodies().length,lastChain=ctx.victims?.length||0;if(lastChain>=20&&sticks>=12&&normalPops>=4&&induced>=25&&emotions>=6&&hi.size>=4&&pops.length>=3&&popColors.size>=6&&start>=30&&end<=10)awardSecret('secret-7',{sticks,normalPops,induced,emotions,highRoles:hi.size,startBodies:start,endBodies:end,lastChain})}
function checkSecretPopRoles(ctx,preGroup,victimSet){const v=ctx.victims||[],snap=ctx.victimSnapshot||[],pre=ctx.preBodyCount||0,post=ctx.postBodyCount??liveBodies().length,colors=new Set(snap.map(x=>x.color)),expr=new Set(snap.map(x=>x.expression)),sizes=new Set(snap.map(x=>x.sizeClass)),decos=new Set(snap.map(x=>x.decoration).filter(x=>x&&x!=='なし')),induced=Math.max(0,v.length-1);
 if(v.length>=30&&induced>=29&&colors.size===6&&post===0)awardSecret('secret-1',{victims:v.length,pre,post});
 const ratio=pre?v.length/pre:0;if(pre>=35&&v.length>=30&&ratio>=.90&&colors.size===6&&expr.size===3&&sizes.size===3&&hasAll(decos,['リボン','メガネ','王冠','芽'])&&post<=3)awardSecret('secret-3',{victims:v.length,pre,post});
 if(post===1){const survivor=liveBodies()[0],sp=survivor?.pocho;if(sp){const lost=ctx.survivorLostBonds?.get(sp.id)||0,age=performance.now()-sp.bornAt;if(pre>=30&&v.length>=29&&age>=20000&&sp.colorsSeen.size>=6&&sp.stickCount>=3&&lost>=2)secretState.pendingSurvivor={id:sp.id,readyAt:performance.now()+5000}}}
 const root=ctx.root?.pocho;if(root){const walls=(root.wall.left>0?1:0)+(root.wall.right>0?1:0)+(root.wall.top>0?1:0),age=performance.now()-root.bornAt;if(root.detachCount>=6&&root.expressionsSeen.size>=3&&root.colorsSeen.size>=6&&root.sizesSeen.size>=3&&walls>=3&&age>=45000&&v.length>=11)awardSecret('secret-5',{victims:v.length,root:root.id});if(root.pushCycleLen>=10&&root.pushCycleColors?.size>=6&&v.length>=16)awardSecret('secret-6',{victims:v.length,cycle:root.pushCycleLen,root:root.id})}
 checkApocalypse(ctx)
}
function periodicSecretChecks(now=performance.now()){
 if(state!=='game'||paused||initializingBoard)return;if(now-secretState.lastBoardSampleAt>=250){secretState.lastBoardSampleAt=now;secretState.bodyTimeline.push({time:now,count:liveBodies().length});while(secretState.bodyTimeline.length&&now-secretState.bodyTimeline[0].time>10500)secretState.bodyTimeline.shift()}
 const ark=arkSnapshot(),base=ark.live.length>=36&&ark.combos.size>=18&&ark.sizes.size>=3&&hasAll(ark.decos,['リボン','メガネ','王冠','芽'])&&ark.minColor>=4;if(base){if(!secretState.arkSince)secretState.arkSince=now}else secretState.arkSince=0;if(base&&now-secretState.arkSince>=3000)awardSecret('secret-2',{bodies:ark.live.length});
 if(secretState.pendingSurvivor){const c=secretState.pendingSurvivor,b=bodyById(c.id);if(!b?.pocho||b.pocho.pendingPop)secretState.pendingSurvivor=null;else if(now>=c.readyAt){awardSecret('secret-4',{survivor:c.id});secretState.pendingSurvivor=null}}
}
...
function spawnPopFx(b,depth=0){const p=document.createElement('div');p.className='pop-burst';p.style.left=(b.position.x/W*100)+'%';p.style.top=(b.position.y/H*100)+'%';p.style.setProperty('--pop-color',b.pocho.hex);p.style.setProperty('--burst-delay',Math.min(depth,6)*.02+'s');scoreLayer.appendChild(p);setTimeout(()=>p.remove(),560)}
function showChainPop(x,y,count){if(performance.now()<secretDisplayUntil)return;const el=document.createElement('div');el.className='chain-pop';const safeX=Math.max(76,Math.min(W-76,x)),safeY=Math.max(82,Math.min(H-90,y-52));el.style.left=(safeX/W*100)+'%';el.style.top=(safeY/H*100)+'%';el.innerHTML=`<span>CHAIN</span><b>×${count}</b>`;scoreLayer.appendChild(el);setTimeout(()=>el.remove(),1050)}

function setFeverUi(show,now=performance.now()){
 const ui=$('#fever-ui'),bar=$('#fever-bar-fill');if(!ui)return;
 ui.classList.toggle('active',!!show);ui.setAttribute('aria-hidden',show?'false':'true');
 if(show&&bar){const left=Math.max(0,feverUntil-now),q=Math.max(0,Math.min(1,left/FEVER_DURATION_MS));bar.style.transform=`scaleX(${q})`}
}
function liveBodies(){return [...bodies].filter(b=>b.pocho?.launched&&!b.pocho.pendingPop)}
function boardGroups(){const live=liveBodies(),liveSet=new Set(live),seen=new Set(),groups=[];for(const b of live){if(seen.has(b))continue;const g=getGroup(b).filter(x=>liveSet.has(x));g.forEach(x=>seen.add(x));groups.push(g)}return groups}
function distinctEventSites(events,minDist=140){const sites=[];for(const e of events){if(!Number.isFinite(e.x)||!Number.isFinite(e.y))continue;if(sites.every(p=>Math.hypot(p.x-e.x,p.y-e.y)>=minDist))sites.push({x:e.x,y:e.y})}return sites.length}
function feverRuleMatch(id,now){
 const live=liveBodies(),groups=boardGroups(),colors=new Set(live.map(b=>b.pocho.color));
 const recent=(ms,type)=>recentEvents.filter(e=>now-e.time<=ms&&(!type||e.type===type));
 if(id==='fever-full-house')return live.length>=18&&groups.filter(g=>g.length>=6).length>=2&&groups.filter(g=>g.length===1).length>=4&&colors.size>=5;
 if(id==='fever-strange-peace'){if(live.length<12)return false;const r=recent(5000);return !r.some(e=>e.type==='くっつく'||e.type==='popChain')&&r.filter(e=>e.type==='何も起こらない').length>=6}
 if(id==='fever-twin-giants'){const big=groups.filter(g=>g.length>=10&&new Set(g.map(b=>b.pocho.color)).size>=3);return big.length>=2}
 if(id==='fever-something-amazing'){const r=recent(1000),contacts=r.filter(e=>e.type==='くっつく'||e.type==='弾ける'||e.type==='何も起こらない');return distinctEventSites(contacts,140)>=3&&r.some(e=>e.type==='くっつく')&&r.some(e=>e.type==='popChain')&&r.some(e=>e.type==='popChain'&&(e.induced||0)>=1)&&r.some(e=>e.type==='表情変化')&&r.some(e=>e.type==='score'&&(e.maxRarity||0)>=4)}
 if(id==='fever-collapse'){const r=recent(1000,'popChain');if(r.length<3)return false;const gone=r.reduce((s,e)=>s+(e.victims||0),0),pre=Math.max(0,...r.map(e=>e.preBodyCount||0)),left=r.some(e=>Number.isFinite(e.x)&&e.x<W/3),center=r.some(e=>Number.isFinite(e.x)&&e.x>=W/3&&e.x<W*2/3),right=r.some(e=>Number.isFinite(e.x)&&e.x>=W*2/3);return pre>=18&&gone>=10&&left&&center&&right&&live.length>=8}
 if(id==='fever-great-gathering'){if(live.length<14)return false;const g=groups.slice().sort((a,b)=>b.length-a.length)[0]||[],decos=new Set(g.map(b=>b.pocho.decoration).filter(x=>x&&x!=='なし')),recentJoins=g.filter(b=>now-(b.pocho.lastGroupJoinAt||0)<=1000).length;return g.length>=14&&g.length/live.length>=.85&&new Set(g.map(b=>b.pocho.color)).size>=5&&new Set(g.map(b=>b.pocho.expression)).size>=3&&decos.size>=3&&recentJoins>=2}
 return false;
}
function spawnFeverCelebration(){
 const party=document.createElement('div');party.className='fever-party';
 const rays=document.createElement('div');rays.className='fever-rays';party.appendChild(rays);
 for(let i=0;i<18;i++){const c=document.createElement('i');c.className='fever-confetti';c.style.setProperty('--x',(2+Math.random()*96)+'%');c.style.setProperty('--dx',(-70+Math.random()*140)+'px');c.style.setProperty('--delay',(Math.random()*.42)+'s');c.style.setProperty('--dur',(1.15+Math.random()*.85)+'s');c.style.setProperty('--rot',(240+Math.random()*720)+'deg');c.style.setProperty('--hue',Math.floor(Math.random()*360));party.appendChild(c)}
 for(let i=0;i<6;i++){const s=document.createElement('b');s.className='fever-spark';s.style.setProperty('--sx',(7+Math.random()*86)+'%');s.style.setProperty('--sy',(12+Math.random()*68)+'%');s.style.setProperty('--sd',(Math.random()*.7)+'s');party.appendChild(s)}
 scoreLayer.appendChild(party);setTimeout(()=>party.remove(),2300);
}
function startFever(rule,now){feverActive=true;feverUntil=now+FEVER_DURATION_MS;feverLastRule=rule.id;feverTriggeredRules.add(rule.id);runStats.fevers=(runStats.fevers||0)+1;save.totals.fevers=(save.totals.fevers||0)+1;document.body.classList.add('fever-active');setFeverUi(true,now);spawnFeverCelebration();const flash=document.createElement('div');flash.className='fever-flash';flash.innerHTML='<span>FEVER!</span><em>×1.5 SCORE</em>';scoreLayer.appendChild(flash);setTimeout(()=>flash.remove(),1500);pushRecent({type:'fever',time:now,rule:rule.id})}
function checkFever(now=performance.now()){
 if(state!=='game'||initializingBoard)return;
 if(feverActive){if(now>=feverUntil){feverActive=false;feverCooldownUntil=now+FEVER_COOLDOWN_MS;document.body.classList.remove('fever-active');setFeverUi(false)}else setFeverUi(true,now);return}
 if(now<feverCooldownUntil)return;
 for(const rule of DATA.feverRules||[]){if(!rule.active||feverTriggeredRules.has(rule.id))continue;if(feverRuleMatch(rule.id,now)){startFever(rule,now);break}}
}
...
function tick(){const emotionNow=performance.now();periodicEmotionChecks(emotionNow);if(state!=='game'||paused)return;const now=performance.now();periodicSecretChecks(now);checkFever(now);confinePrelaunchSet();wallChecks();for(const b of bodies){if(!b.pocho?.launched)continue;const p=b.pocho,dt=Math.max(0,(now-p.lastUpdate)/1000);p.lastUpdate=now;p.distance+=speed(b)*dt*60;p.maxSpeed=Math.max(p.maxSpeed,speed(b));if(speed(b)>=T.HIGH)p.highSpeedSeen=true;Body.applyForce(b,b.position,{x:0,y:-b.mass*PHYS.UP_FORCE});const m=speed(b);if(m>PHYS.MAX_SPEED){const s=PHYS.MAX_SPEED/m;Body.setVelocity(b,{x:b.velocity.x*s,y:b.velocity.y*s})}if(Math.abs(b.angularVelocity)>PHYS.MAX_ANG)Body.setAngularVelocity(b,Math.sign(b.angularVelocity)*PHYS.MAX_ANG);if(m<T.STILL){if(!p.stillSince)p.stillSince=now;if(now-p.stillSince>5000)p.longStill=true}else p.stillSince=0;}
 ...
 if(remaining===0&&!currentSet){
  // v0.2.3: 通常は静止を待つが、微小振動や柔らかいジョイントで永久待機しないよう最終射出から8秒で必ず終了。
  if(finalShotAt&&now-finalShotAt>=FINAL_WATCH_MAX_MS){gameStatus.textContent='見届け時間終了';finishGame();return}
  const pending=[...bodies].some(b=>b.pocho?.pendingPop);const active=pending||[...bodies].some(b=>speed(b)>1.1);
  if(!active&&now-lastActivity>1200){if(!settleSince)settleSince=now;if(now-settleSince>1500){finishGame();return}}else settleSince=0;
 }
}
function drawOverlay(){if(!render)return;const ctx=render.context;ctx.save();const zt=launchZoneTop();ctx.fillStyle='#f7f3ecb8';ctx.fillRect(0,zt,W,H-zt);ctx.strokeStyle='#9c8f7f80';ctx.setLineDash([7,7]);ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(0,zt);ctx.lineTo(W,zt);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle='#766d63a8';ctx.font='600 12px system-ui';ctx.textAlign='left';ctx.fillText('射出ゾーン',12,zt+20);ctx.strokeStyle='#ffffffaa';ctx.lineWidth=2;ctx.strokeRect(1,1,W-2,H-2);for(const c of bonds){if(!c.bodyA?.pocho||!c.bodyB?.pocho)continue;const ax=c.bodyA.position.x,ay=c.bodyA.position.y,bx=c.bodyB.position.x,by=c.bodyB.position.y,mx=(ax+bx)/2,my=(ay+by)/2;ctx.lineCap='round';ctx.lineJoin='round';// soft outer gel bridge
ctx.strokeStyle='#fffaf0b8';ctx.lineWidth=13;ctx.beginPath();ctx.moveTo(ax,ay);ctx.lineTo(bx,by);ctx.stroke();// inner seam makes the connection readable even on pale bodies
ctx.strokeStyle='#8c81705f';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(ax,ay);ctx.lineTo(bx,by);ctx.stroke();ctx.fillStyle='#fffaf0d8';ctx.beginPath();ctx.arc(mx,my,4.5,0,Math.PI*2);ctx.fill();}if(currentSet&&!currentSet.launched){for(const c of currentSet.links){ctx.lineWidth=5;ctx.lineCap='round';ctx.strokeStyle='#8e817151';ctx.beginPath();ctx.moveTo(c.bodyA.position.x,c.bodyA.position.y);ctx.lineTo(c.bodyB.position.x,c.bodyB.position.y);ctx.stroke()}}for(const b of bodies)if(b.pocho){let rr=b.pocho.radius;if(b.pocho.pendingPop){const t=performance.now(),start=b.pocho.popWarningStart||t,end=b.pocho.popAt||t+1,q=Math.max(0,Math.min(1,(t-start)/Math.max(1,end-start))),pulse=.05+.13*q+.045*Math.sin(q*Math.PI*5);rr*=1+pulse;ctx.save();ctx.globalAlpha=.34+.28*q;ctx.strokeStyle='#fff';ctx.lineWidth=4+5*q;ctx.beginPath();ctx.arc(b.position.x,b.position.y,rr+5+8*q,0,Math.PI*2);ctx.stroke();ctx.restore()}drawPocho2D(ctx,b.position.x,b.position.y,rr,b.pocho,b.angle)};if(dragging&&grabConstraint&&grabbed){ctx.strokeStyle='#846f59a0';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(grabConstraint.pointA.x,grabConstraint.pointA.y);ctx.lineTo(grabbed.position.x,grabbed.position.y);ctx.stroke();ctx.beginPath();ctx.arc(grabConstraint.pointA.x,grabConstraint.pointA.y,8,0,Math.PI*2);ctx.stroke()}ctx.restore()}
...
function jstParts(){const d=new Date(Date.now()+9*3600000);return {date:d.toISOString().slice(0,10),month:d.toISOString().slice(0,7)}}
function isoWeekJst(){const d=new Date(Date.now()+9*3600000),x=new Date(Date.UTC(d.getUTCFullYear(),d.getUTCMonth(),d.getUTCDate()));x.setUTCDate(x.getUTCDate()+4-(x.getUTCDay()||7));const y=new Date(Date.UTC(x.getUTCFullYear(),0,1));const w=Math.ceil((((x-y)/86400000)+1)/7);return `${x.getUTCFullYear()}-W${String(w).padStart(2,'0')}`}
function weeklyChoice(weekId){const metrics=['maxChain','maxSingleScore','maxFever','maxGroupSize','maxRoles','maxSticks','maxPops'],modes=['short','middle','long'];let h=2166136261;for(const ch of weekId){h^=ch.charCodeAt(0);h=Math.imul(h,16777619)}h>>>=0;return {metric:metrics[h%metrics.length],mode:modes[Math.floor(h/metrics.length)%modes.length]}}
function recordRun(final){const now=jstParts(),r=save.records||(save.records=defaultSave().records);if(r.daily.date!==now.date)r.daily={date:now.date,best:{short:0,middle:0,long:0},bestAt:{short:null,middle:null,long:null}};if(r.monthly.month!==now.month)r.monthly={month:now.month,best:{short:0,middle:0,long:0},bestAt:{short:null,middle:null,long:null}};const stamp=new Date().toISOString();if(final>(r.daily.best[modeKey]||0)){r.daily.best[modeKey]=final;r.daily.bestAt[modeKey]=stamp}if(final>(r.monthly.best[modeKey]||0)){r.monthly.best[modeKey]=final;r.monthly.bestAt[modeKey]=stamp}if(final>(save.best[modeKey]||0)||!r.bestAt[modeKey])r.bestAt[modeKey]=stamp;const sp=r.specialBest[modeKey]||(r.specialBest[modeKey]=defaultSpecial());sp.maxChain=Math.max(sp.maxChain||0,runStats.maxChain||0);sp.maxSingleScore=Math.max(sp.maxSingleScore||0,runStats.maxSingle||0);sp.maxFever=Math.max(sp.maxFever||0,runStats.fevers||0);sp.maxGroupSize=Math.max(sp.maxGroupSize||0,runStats.maxGroupSize||0);sp.maxRoles=Math.max(sp.maxRoles||0,runStats.rolesTriggered||0);sp.maxSticks=Math.max(sp.maxSticks||0,runStats.sticks||0);sp.maxPops=Math.max(sp.maxPops||0,runStats.pops||0);const weekId=isoWeekJst(),wc=weeklyChoice(weekId);if(r.weekly.weekId!==weekId||r.weekly.mode!==wc.mode||r.weekly.metric!==wc.metric)r.weekly={weekId,mode:wc.mode,metric:wc.metric,bestValue:0,bestAt:null};if(modeKey===wc.mode){const values={maxChain:runStats.maxChain||0,maxSingleScore:runStats.maxSingle||0,maxFever:runStats.fevers||0,maxGroupSize:runStats.maxGroupSize||0,maxRoles:runStats.rolesTriggered||0,maxSticks:runStats.sticks||0,maxPops:runStats.pops||0};const v=values[wc.metric]||0;if(v>(r.weekly.bestValue||0)){r.weekly.bestValue=v;r.weekly.bestAt=stamp}}}
function finishGame(){if(state!=='game')return;feverActive=false;document.body.classList.remove('fever-active');setFeverUi(false);const final=score,old=save.best[modeKey]||0,isRecord=final>old;if(isRecord){save.best[modeKey]=final;(save.records||(save.records=defaultSave().records)).bestAt[modeKey]=new Date().toISOString()}save.plays[modeKey]=(save.plays[modeKey]||0)+1;recordRun(final);persist();const detail={mode:modeKey,score:final,isRecord,run:{contacts:runStats.contacts,sticks:runStats.sticks,pops:runStats.pops,induced:runStats.induced,maxRarity:runStats.maxRarity,maxSingle:runStats.maxSingle,rolesTriggered:runStats.rolesTriggered,shots:runStats.shots,fevers:runStats.fevers,maxChain:runStats.maxChain,maxGroupSize:runStats.maxGroupSize}};teardownPhysics();showResult(final,isRecord);window.dispatchEvent(new CustomEvent('pocho:run-finished',{detail}))}
function showResult(final,isRecord){showScreen('result');$('#result-score').textContent=final.toLocaleString();$('#result-record').textContent=isRecord?'NEW RECORD!':'';const note=$('#result-login-note');if(note)note.classList.toggle('hidden',!!save.profile?.playerId);const d=$('#result-details');d.innerHTML='';const arr=[['MODE',MODES[modeKey].label],['最高レア',runStats.maxRarity?'★'+runStats.maxRarity:'なし'],['発生役',runStats.rolesTriggered],['NEW',runStats.newRoles.size],['最大単発',runStats.maxSingle.toLocaleString()],['誘爆',runStats.induced],['FEVER',runStats.fevers||0]];for(const [a,b] of arr){const x=document.createElement('div');x.className='result-detail';x.innerHTML=`<small>${a}</small><b>${b}</b>`;d.appendChild(x)}}

function renderBook(){const list=$('#book-list'),normal=DATA.roles.filter(r=>r.active&&r.rarity<=5),secrets=(DATA.secretRoles||[]).filter(r=>r.active&&save.roles[String(r.id)]?.discovered),active=[...normal,...secrets],found=active.filter(r=>save.roles[String(r.id)]?.discovered).length;$('#book-summary').textContent=`発見 ${found} / ${active.length}`;list.innerHTML='';for(const r of active){const rec=save.roles[String(r.id)],found=!!rec?.discovered,card=document.createElement('div');card.className='role-card '+(found?'':'locked')+(r.rarity===6?' secret-role-card':'');card.innerHTML=`<div class="role-top"><span class="stars rarity-${r.rarity}">${'★'.repeat(r.rarity)}</span><span class="role-name">${found?escapeHtml(r.name):'？？？？？？'}</span></div><p>${escapeHtml(found?r.discovered:r.undiscovered)}</p>${found?`<div class="role-meta"><span>発見 ${rec.count}回</span><span>最高 +${rec.best.toLocaleString()}</span></div>`:''}`;list.appendChild(card)} }
...
window.POCHO_APP={getSave:()=>JSON.parse(JSON.stringify(save)),replaceSave:(v)=>{save=normalizeSave(v);persist();renderBook();renderStats();if(state==='result'){const note=$('#result-login-note');if(note)note.classList.toggle('hidden',!!save.profile?.playerId)}},persist,renderBook,renderStats,drawPocho2D,colors:COLORS,expressions:EXPRESSIONS,decorations:DECOS,modes:MODES,weeklyChoice,isoWeekJst,jstParts};
