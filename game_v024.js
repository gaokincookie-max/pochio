...
function periodicEmotionChecks(now){for(const b of bodies){if(!b.pocho?.launched||now-b.pocho.lastEmotionCheck<500)continue;b.pocho.lastEmotionCheck=now;runEmotionRules('periodic',{time:now},[b])}}
...
function popGroup(origin,ctx){
 ...
 for(const b of victims){spawnPopFx(b);Composite.remove(engine.world,b);bodies.delete(b)}lastActivity=now;ctx.time=now;runEmotionRules('pop',ctx,[...failed,...nearWitness]);
}
...
function tick(){const emotionNow=performance.now();periodicEmotionChecks(emotionNow);if(state!=='game'||paused)return;const now=performance.now();confinePrelaunchSet();wallChecks();for(const b of bodies){if(!b.pocho?.launched)continue;const p=b.pocho,dt=Math.max(0,(now-p.lastUpdate)/1000);p.lastUpdate=now;p.distance+=speed(b)*dt*60;p.maxSpeed=Math.max(p.maxSpeed,speed(b));if(speed(b)>=T.HIGH)p.highSpeedSeen=true;Body.applyForce(b,b.position,{x:0,y:-b.mass*PHYS.UP_FORCE});const m=speed(b);if(m>PHYS.MAX_SPEED){const s=PHYS.MAX_SPEED/m;Body.setVelocity(b,{x:b.velocity.x*s,y:b.velocity.y*s})}if(Math.abs(b.angularVelocity)>PHYS.MAX_ANG)Body.setAngularVelocity(b,Math.sign(b.angularVelocity)*PHYS.MAX_ANG);if(m<T.STILL){if(!p.stillSince)p.stillSince=now;if(now-p.stillSince>5000)p.longStill=true}else p.stillSince=0;}
 ...
 if(remaining===0&&!currentSet){
  ...
  const active=[...bodies].some(b=>speed(b)>1.1);
