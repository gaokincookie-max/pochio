...
function startGame(m){
 ...
 runStats={contacts:0,sticks:0,pops:0,induced:0,newRoles:new Set(),maxRarity:0,maxSingle:0,rolesTriggered:0,shots:0,fevers:0};
...
function spawnFeverCelebration(){
 ...
 for(let i=0;i<42;i++){const c=document.createElement('i');c.className='fever-confetti';c.style.setProperty('--x',(2+Math.random()*96)+'%');c.style.setProperty('--dx',(-70+Math.random()*140)+'px');c.style.setProperty('--delay',(Math.random()*.42)+'s');c.style.setProperty('--dur',(1.15+Math.random()*.85)+'s');c.style.setProperty('--rot',(240+Math.random()*720)+'deg');c.style.setProperty('--hue',Math.floor(Math.random()*360));party.appendChild(c)}
 for(let i=0;i<12;i++){const s=document.createElement('b');s.className='fever-spark';s.style.setProperty('--sx',(7+Math.random()*86)+'%');s.style.setProperty('--sy',(12+Math.random()*68)+'%');s.style.setProperty('--sd',(Math.random()*.7)+'s');party.appendChild(s)}
 scoreLayer.appendChild(party);setTimeout(()=>party.remove(),2300);
}
function startFever(rule,now){feverActive=true;feverUntil=now+FEVER_DURATION_MS;feverLastRule=rule.id;feverTriggeredRules.add(rule.id);runStats.fevers=(runStats.fevers||0)+1;document.body.classList.add('fever-active');setFeverUi(true,now);spawnFeverCelebration();const flash=document.createElement('div');flash.className='fever-flash';flash.innerHTML='<span>FEVER!</span><em>×1.5 SCORE</em>';scoreLayer.appendChild(flash);setTimeout(()=>flash.remove(),1500);pushRecent({type:'fever',time:now,rule:rule.id})}
