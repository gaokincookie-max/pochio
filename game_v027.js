...
function popGroup(origin,ctx){
 ...
 pushRecent({type:'popChain',time:now,victims:victims.length,induced:Math.max(0,victims.length-1),preBodyCount:[...bodies].filter(x=>x.pocho&&!x.pocho.pendingPop).length+victims.length,rootId:origin.pocho.id});
...
function feverRuleMatch(id,now){
 ...
 if(id==='fever-something-amazing'){const r=recent(1000);return r.some(e=>e.type==='くっつく')&&r.some(e=>e.type==='popChain')&&r.some(e=>e.type==='popChain'&&(e.induced||0)>=1)&&r.some(e=>e.type==='表情変化')&&r.some(e=>e.type==='score'&&(e.maxRarity||0)>=3)}
 if(id==='fever-collapse'){const r=recent(1500,'popChain');if(r.length<2)return false;const gone=r.reduce((s,e)=>s+(e.victims||0),0),pre=Math.max(0,...r.map(e=>e.preBodyCount||0));return pre>=12&&gone>=5&&live.length>=7}
 if(id==='fever-great-gathering'){if(live.length<10)return false;const g=groups.slice().sort((a,b)=>b.length-a.length)[0]||[];return g.length>=10&&g.length/live.length>=.70&&new Set(g.map(b=>b.pocho.color)).size>=4&&new Set(g.map(b=>b.pocho.expression)).size>=3&&g.some(b=>now-(b.pocho.lastGroupJoinAt||0)<=2000)}
 return false;
}
