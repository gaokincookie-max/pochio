...
function feverRuleMatch(id,now){
 ...
 if(id==='fever-full-house')return live.length>=15&&groups.filter(g=>g.length>=5).length>=2&&groups.filter(g=>g.length===1).length>=3&&colors.size>=3;
 ...
 if(id==='fever-collapse'){const r=recent(1000,'popChain');if(r.length<3)return false;const gone=r.reduce((s,e)=>s+(e.victims||0),0),pre=Math.max(0,...r.map(e=>e.preBodyCount||0)),left=r.some(e=>Number.isFinite(e.x)&&e.x<W/2),right=r.some(e=>Number.isFinite(e.x)&&e.x>=W/2);return pre>=16&&gone>=8&&left&&right&&live.length>=6}
 if(id==='fever-great-gathering'){if(live.length<12)return false;const g=groups.slice().sort((a,b)=>b.length-a.length)[0]||[],decos=new Set(g.map(b=>b.pocho.decoration).filter(x=>x&&x!=='なし')),recentJoins=g.filter(b=>now-(b.pocho.lastGroupJoinAt||0)<=1000).length;return g.length>=12&&g.length/live.length>=.80&&new Set(g.map(b=>b.pocho.color)).size>=5&&new Set(g.map(b=>b.pocho.expression)).size>=3&&decos.size>=2&&recentJoins>=2}
 return false;
}
function startFever(rule,now){feverActive=true;feverUntil=now+FEVER_DURATION_MS;feverLastRule=rule.id;feverTriggeredRules.add(rule.id);runStats.fevers=(runStats.fevers||0)+1;document.body.classList.add('fever-active');setFeverUi(true,now);const flash=document.createElement('div');flash.className='fever-flash';flash.innerHTML='<span>FEVER!</span>';scoreLayer.appendChild(flash);setTimeout(()=>flash.remove(),1150);pushRecent({type:'fever',time:now,rule:rule.id})}
