...
function persist(){try{save.meta.updatedAt=new Date().toISOString();save.meta.lastGameVersion='0.3.2';localStorage.setItem(SAVE_KEY,JSON.stringify(save));window.dispatchEvent(new CustomEvent('pocho:save-changed',{detail:{save:JSON.parse(JSON.stringify(save))}}))}catch{}}
...
function startGame(m){
 stopGame();const session=++gameSessionId;modeKey=m;score=0;remaining=MODES[m].shots;shotIndex=0;lastActivity=performance.now();settleSince=0;finalShotAt=0;gameStartedAt=performance.now();recentEvents=[];feverActive=false;feverUntil=0;feverCooldownUntil=0;feverTriggeredRules=new Set();feverLastRule=null;setFeverUi(false);
...
function popGroup(origin,ctx){
 ...
 pushRecent({type:'popChain',time:now,x:origin.position.x,y:origin.position.y,victims:victims.length,induced:Math.max(0,victims.length-1),preBodyCount:[...bodies].filter(x=>x.pocho&&!x.pocho.pendingPop).length+victims.length,rootId:origin.pocho.id});
 lastActivity=now;ctx.time=now;runEmotionRules('pop',ctx,[...failed,...nearWitness]);checkFever(now);
}
...
function tick(){const emotionNow=performance.now();periodicEmotionChecks(emotionNow);if(state!=='game'||paused)return;const now=performance.now();checkFever(now);confinePrelaunchSet();wallChecks();for(const b of bodies){if(!b.pocho?.launched)continue;const p=b.pocho,dt=Math.max(0,(now-p.lastUpdate)/1000);p.lastUpdate=now;p.distance+=speed(b)*dt*60;p.maxSpeed=Math.max(p.maxSpeed,speed(b));if(speed(b)>=T.HIGH)p.highSpeedSeen=true;Body.applyForce(b,b.position,{x:0,y:-b.mass*PHYS.UP_FORCE});const m=speed(b);if(m>PHYS.MAX_SPEED){const s=PHYS.MAX_SPEED/m;Body.setVelocity(b,{x:b.velocity.x*s,y:b.velocity.y*s})}if(Math.abs(b.angularVelocity)>PHYS.MAX_ANG)Body.setAngularVelocity(b,Math.sign(b.angularVelocity)*PHYS.MAX_ANG);if(m<T.STILL){if(!p.stillSince)p.stillSince=now;if(now-p.stillSince>5000)p.longStill=true}else p.stillSince=0;}
